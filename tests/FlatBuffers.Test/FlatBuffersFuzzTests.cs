...
namespace Google.FlatBuffers.Test
{
  ...
  public class FlatBuffersFuzzTests
  {
    ...
    public void TestNumbers()
    {
      ...
      Assert.ArrayEqual(new byte[] { 0, 0, 0x22, 0x82, 0, 255, 129, 1 },
        builder.DataBuffer.ToFullArray()); // Second pad
      builder.AddUshort(0xFEEE);
      Assert.ArrayEqual(new byte[] { 0xEE, 0xFE, 0x22, 0x82, 0, 255, 129, 1 },
        builder.DataBuffer.ToFullArray()); // no pad
      builder.AddInt(-53687092);
      Assert.ArrayEqual(new byte[] { 0, 0, 0, 0, 204, 204, 204, 252, 0xEE, 0xFE, 0x22, 0x82, 0, 255, 129, 1 },
        builder.DataBuffer.ToFullArray()); // third pad
      builder.AddUint(0x98765432);
      Assert.ArrayEqual(
        new byte[] { 0x32, 0x54, 0x76, 0x98, 204, 204, 204, 252, 0xEE, 0xFE, 0x22, 0x82, 0, 255, 129, 1 },
        builder.DataBuffer.ToFullArray()); // no pad
    }
    ...
    public void TestNumbers64()
    {
      ...
      Assert.ArrayEqual(new byte[] { 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11 },
    ...
    public void TestCreateAsciiString()
    {
      ...
      Assert.ArrayEqual(new byte[] { 3, 0, 0, 0, (byte)'f', (byte)'o', (byte)'o', 0 },
        builder.DataBuffer.ToFullArray());
    ...
    public void TestBunchOfBools()
    {
      ...
      for (var i = 0; i < 8; i++) builder.AddBool(i, true, false);
