...
namespace Google.FlatBuffers.Test
{
  ...
  public class FlatBuffersExampleTests
  {
    ...
    private void CanCreateNewFlatBufferFromScratch(bool sizePrefix)
    {
      ...
      for (int i = 4; i >= 0; i--) fbb.AddByte((byte)i);
      ...
      var monster = Monster.GetRootAsMonster(dataBuffer);
      ...
      for (int i = 0; i < monster.InventoryLength; i++) Assert.AreEqual(monster.Inventory(i), i + 1);
    ...
    private void TestBuffer(ByteBuffer bb)
    {
      ...
      var invsum = 0;
      for (var i = 0; i < monster.InventoryLength; i++) invsum += monster.Inventory(i);
      ...
      foreach (var inv in inventoryArray) invsum -= inv;
    ...
    public void TestFixedLenghtArrays()
    {
      ...
      sbyte c;
      int[,] d_a = new int[2, 2];
      var d_b = new TestEnum[2];
      var d_c = new TestEnum[2, 2];
      ...
      var arrayOffset = ArrayStruct.CreateArrayStruct(
        builder, a, b, c, d_a, d_b, d_c, d_d, e, f);

      // Create a table with the ArrayStruct.
      ArrayTable.StartArrayTable(builder);
      ArrayTable.AddA(builder, arrayOffset);
      var tableOffset = ArrayTable.EndArrayTable(builder);

      ArrayTable.FinishArrayTableBuffer(builder, tableOffset);

      var table = ArrayTable.GetRootAsArrayTable(builder.DataBuffer);
    ...
    private void AreEqual(Monster a, MonsterT b)
    {
      ...
      for (var i = 0; i < a.InventoryLength; ++i) Assert.AreEqual(a.Inventory(i), b.Inventory[i]);
      ...
      for (var i = 0; i < inventoryArrayLength; ++i) Assert.AreEqual(inventoryArray[i], b.Inventory[i]);
      ...
      for (var i = 0; i < a.TestarrayofboolsLength; ++i) Assert.AreEqual(a.Testarrayofbools(i), b.Testarrayofbools[i]);

      Assert.AreEqual(a.VectorOfLongsLength, b.VectorOfLongs.Count);
      for (var i = 0; i < a.VectorOfLongsLength; ++i) Assert.AreEqual(a.VectorOfLongs(i), b.VectorOfLongs[i]);

      Assert.AreEqual(a.VectorOfDoublesLength, b.VectorOfDoubles.Count);
      for (var i = 0; i < a.VectorOfDoublesLength; ++i) Assert.AreEqual(a.VectorOfDoubles(i), b.VectorOfDoubles[i]);

      Assert.AreEqual(a.VectorOfEnumsLength, b.VectorOfEnums.Count);
      for (var i = 0; i < a.VectorOfEnumsLength; ++i) Assert.AreEqual(a.VectorOfEnums(i), b.VectorOfEnums[i]);
    }
    ...
    private void AreEqual(Monster a, Monster b)
    {
      ...
      for (var i = 0; i < inventoryArrayALength; ++i) Assert.AreEqual(inventoryArrayA[i], inventoryArrayB[i]);
    ...
    private void AreEqual(ArrayTable a, ArrayTableT b)
    {
      ...
      for (int i = 0; i < 15; ++i) Assert.AreEqual(a.A.Value.B(i), b.A.B[i]);
      ...
      for (int i = 0; i < 2; ++i)
      {
        ...
        for (int j = 0; j < 2; ++j) Assert.AreEqual(ad.A(j), bd.A[j]);

        Assert.AreEqual(ad.B, bd.B);

        for (int j = 0; j < 2; ++j) Assert.AreEqual(ad.C(j), bd.C[j]);

        for (int j = 0; j < 2; ++j) Assert.AreEqual(ad.D(j), bd.D[j]);
      }

      Assert.AreEqual(a.A.Value.E, b.A.E);

      for (int i = 0; i < 2; ++i) Assert.AreEqual(a.A.Value.F(i), b.A.F[i]);
    }
    ...
    private void AreEqual(ArrayTable a, ArrayTable b)
    {
      ...
      for (int i = 0; i < 15; ++i) Assert.AreEqual(a.A.Value.B(i), b.A.Value.B(i));
      ...
      for (int i = 0; i < 2; ++i) Assert.AreEqual(a.A.Value.F(i), b.A.Value.F(i));
    }
    ...
    private void AreEqual(Movie a, Movie b)
    {
      ...
      Assert.AreEqual(a.Characters<Attacker>(0).Value.SwordAttackDamage,
        b.Characters<Attacker>(0).Value.SwordAttackDamage);
    ...
    private static void KeepComparing(Monster mon, int count, float floatValue, double doubleValue)
    {
      int i = 0;
      while (++i <= count)
      {
        Interlocked.Add(ref _comparisons, 1);
        if (mon.Pos.Value.Test1 != doubleValue || mon.Pos.Value.Z != floatValue) Interlocked.Add(ref _failures, 1);
      }
    }
    ...
    {
      Assert.AreEqual((int)@public.NONE, 0);

      Assert.AreEqual((int)ABC.@void, 0);
      Assert.AreEqual((int)ABC.where, 1);
      Assert.AreEqual((int)ABC.@stackalloc, 2);

      var fbb = new FlatBufferBuilder(1);
      var offset = KeywordsInTable.CreateKeywordsInTable(
        fbb, ABC.@stackalloc, @public.NONE);
      fbb.Finish(offset.Value);

      var keywordsInTable =
        KeywordsInTable.GetRootAsKeywordsInTable(fbb.DataBuffer);

      Assert.AreEqual(keywordsInTable.Is, ABC.@stackalloc);
      Assert.AreEqual(keywordsInTable.Private, @public.NONE);
    }
